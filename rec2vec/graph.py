...
def records_to_graph():
    ...
    for key, value in movies.iteritems():
      tempNode = Node(getNextnId(),value.name,'movie')
      nodedict[key] = tempNode
    ...
    for user in ratings:
      tempNode = Node(getNextnId(), user,'user')
      nodedict[user] = tempNode
      nodelist.append(tempNode)

    for actor in list(actors):
      tempNode = Node(getNextnId(),actor,'actor')
      nodedict[actor] = tempNode
      nodelist.append(tempNode)

    for director in list(directors):
      tempNode = Node(getNextnId(),director,'director')
      nodedict[director] = tempNode
      nodelist.append(tempNode)

    for genre in list(genres):
      tempNode = Node(getNextnId(),genre,'genre')
    ...
    for k,v in movies.iteritems():
      ...
      if movies[k].director is not None:
        directorNode = nodedict[v.director]
